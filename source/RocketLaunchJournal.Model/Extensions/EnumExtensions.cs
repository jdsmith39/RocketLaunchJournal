...
public static class EnumExtensions
{
    ...
    /// <summary>
    /// returns the Enum value name, if it has a display attribute with a short name it returns that instead.
    /// </summary>
    /// <param name="en">Enum</param>
    /// <returns>string name</returns>
    public static string GetShortDisplayName(this Enum en)
    {
        var display = en.GetType().GetMember(en.ToString()).First().GetCustomAttributes(false).OfType<DisplayAttribute>().LastOrDefault();
        return display?.GetShortName() ?? en.ToString();
    }
